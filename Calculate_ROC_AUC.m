...
for i = 1:(length(H1_index)-2+length(H0_index)-2)
    ...
    Test_Statistic1(i) = (x_filtered*Hc1*theta_star)/(x_filtered*x_filtered'); % LAD-LRT - Moving
...
thre1     = linspace(min_stat1,max_stat1,N_thre); 

min_stat2 = min(Test_Statistic2);
max_stat2 = max(Test_Statistic2);
thre2     = linspace(min_stat2,max_stat2,N_thre);

for i = 1:(length(H1_index)-2+length(H0_index)-2)
    disp(['i= ',num2str(i)]);     
    for j = 1:N_thre
        if Test_Statistic1(i) >= thre1(j)
...
grid on;
axis([0 1 0 1]);
hl = legend('LAD-LRT-Equally spaced','LS-LRT-Equally spaced','LS-LRT');
hx = xlabel('$P_{\rm{FA}}$');
hy = ylabel('$P_{\rm{D}}$');
...
ppf(3).LineWidth=2;
grid on     
hl = legend(['LAD-LRT-Equally spaced AUC=' num2str(AUC1)],...
    ['LS-LRT-Equally spaced AUC=' num2str(AUC2)],...
    ['LS-LRT AUC=' num2str(AUC3)],'location', 'southeast');
hx = xlabel('$P_{\rm{FA}}$');
