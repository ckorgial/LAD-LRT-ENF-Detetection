...
for i = 1:(length(H1_index)-2+length(H0_index)-2)
    ...
    for j = 1:length(duration)
        ...
        Test_Statistic1 = (x_filtered*Hc1*theta_star)/(x_filtered*x_filtered'); % LAD-LRT - Median
...
figure(1);
pf=plot(duration,ACC1*100,'bo-',duration,ACC2*100,'gx:',duration,ACC3*100,'r+-.');
pf(1).LineWidth=2;
pf(2).LineWidth=2;
pf(3).LineWidth=2;

grid on;
hl = legend('LAD-LRT-Median','LS-LRT-Median','LS-LRT');
hx = xlabel('$N/f_{\rm{S}}$ (seconds)');
hy = ylabel('Accuracy ($\%$)');
